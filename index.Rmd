...
ascicat <- reactive({
  ...
  out <- ascidat %>% 
    ...
    mutate(
      scrcat = ifelse(scr < thrsh, 'not met', 'met')
    )
...
```{r}
selectInput('thr', 'Select biointegrity goal:', choices = list('1% reference' = 0.01, '10%  reference' = 0.1, '30%  reference' = 0.3), selected = '0.1')
```
...
renderPlot({
  ...
  p <- ggplot(toplo, aes(x = sit2, y = scr)) + 
    ...
    geom_hline(aes(yintercept = thrsh), linetype = 'dashed', colour = 'tomato1', size = 1) +
    facet_grid(ind ~ tax) +
