...
names(evvar) <- c('Phosphorus (mg/L)', 'Total Nitrogen (mg/L)', '% Shading', '% Sands and Fines', 'Conductivity (mS/cm)', 'Temperature (C)', 'Mean Slope of Reach (%)', 'log-Watershed Area (m2)', 'Elevation (m)')
...
output$evplo <- renderPlot({
  ...
  pnew <- ggplot(toplo_new, aes_string(x = evvr, y = 'scrnew')) +
    ...
    scale_y_continuous('ASCI score', expand = c(0, 0), limits = c(0, ymax)) +  
    scale_x_continuous(xlab) +
    pbase
